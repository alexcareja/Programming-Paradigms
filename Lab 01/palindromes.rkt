...
(define (how-many-palindromes? n Bases)
  (if (null? Bases)
      0
      (if (palindrome? (num->base n (car Bases)))
          (+ 1 (how-many-palindromes? n (cdr Bases)))
          (+ 0 (how-many-palindromes? n (cdr Bases))))))

(define (first-b-pal start b)
  (if (>= (how-many-palindromes? start (list 2 3 4 5 6 7 8 9 10)) b)
      start
...
(define (calc-max-length-pal list)
  (if (null? list)
      0
      (max (if (palindrome? (list->num list) 10)
               (length list)
               0) (calc-max-length-pal (cdr list)))))

(define (get-length-pal list len)
  (if (null? list)
      0
      (if (palindrome? (list->num list) 10)
               (if (equal? len (length list))
                   (list->num list)
                   0)
               (calc-max-length-pal (cdr list)))))

(define (longest-palindrome-length n)
   (if (zero? n)
       0
       (max (calc-max-length-pal (num->list n)) (longest-palindrome-length (list->num (cdr (num->list n)))))))

(define (longest-palindrome n)
   (if (zero? n)
       '()
       (append (get-length-pal n) (longest-palindrome(list->num (cdr (num->list n)))))))
